...
def cleanlinked(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        if param == "on" or param == "true" or param == "yes" or param == "On" or param == "Yes" or param == "True":
            ...
            msg.reply_text(f"*Enabled* cleanlinked in {chat.title}. Messages sent by channel will be deleted.", parse_mode=ParseMode.MARKDOWN)
            return
        elif param == "off" or param == "false" or param == "no" or param == "No" or param == "Off" or param == "False":
            acm_sql.setCleanLinked(chat.id, False)
            msg.reply_text(f"*Disabled* cleanlinked in {chat.title}.", parse_mode=ParseMode.MARKDOWN)
            return
        else:
...
CLEANLINKED_HANDLER = CommandHandler(['antichannel', 'cleanlinked'], cleanlinked, filters=Filters.chat_type.groups, run_async=True)
