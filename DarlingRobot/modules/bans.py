...
from DarlingRobot.modules.helper_funcs.extraction import extract_user_and_text
from DarlingRobot.modules.helper_funcs.string_handling import extract_time
from DarlingRobot.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("What? Are you noob or something?, He is my Darling you fool")
        elif user_id in DEV_USERS:
            message.reply_text("I can't act against our Councils.")
        elif user_id in DRAGONS:
            message.reply_text(
                "Fighting this Mayor here will put user lives at risk."
            )
        elif user_id in DEMONS:
            message.reply_text(
                "Bring an order from Councils to fight a member of The 9's."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from Mayors to fight Specials."
            )
        elif user_id in WOLVES:
            message.reply_text("Commander access make them ban immune!")
        else:
        ...
        bot.sendMessage(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="×͜× Unban ×͜×", callback_data=f"unbanb_unban={user_id}"
                        ),
...
def punchme(update: Update, context: CallbackContext):
    ...
    if res:
        update.effective_message.reply_text(
            "Bye Bye, Take a good care of your baby!",
        )
    else:
