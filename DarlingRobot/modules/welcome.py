...
from DarlingRobot.modules.helper_funcs.handlers import MessageHandlerChecker
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    f"Welcome to {html.escape(chat.title)} Darling ꨄ.", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"My Darling just joined the chat. ꨄ"
                )
                continue
            ...
            if new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Whoa! The Council Minister just joined!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DRAGONS:
                update.effective_message.reply_text(
                    "Huh! A Mayor just joined! Stay Alert!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DEMONS:
                update.effective_message.reply_text(
                    "Huh! Someone from The 9's just joined!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in WOLVES:
                update.effective_message.reply_text(
                    "Oof! A Special User just joined!", reply_to_message_id=reply
                )
                continue
