...
import DarlingRobot.modules.sql.welcome_sql as sql
import DarlingRobot
...
from DarlingRobot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from DarlingRobot.modules.helper_funcs.msg_types import get_welcome_type
from DarlingRobot.modules.helper_funcs.string_handling import (
...
DARLING_IMG = "https://telegra.ph/file/2ab6574a3c9dcf13bdf50.jpg"
COUNCIL_IMG = "https://telegra.ph/file/9cf4f56ef817449092894.jpg"
MAYOR_IMG = "https://telegra.ph/file/a13dbfa12424dc323a769.jpg"
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_video(
                    DARLING_IMG, caption=f"OwO!!! My Darling {html.escape(user.first_name)} Just Joined this group", reply_to_message_id=reply,
                    parse_mode=ParseMode.HTML,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Zero's Darling just joined the group"
                )
                continue
            ...
            # Welcome Devs
                ...
                update.effective_message.reply_video(
                    COUNCIL_IMG, caption = f"Be Cool! A Council Minister{html.escape(user.first_name)} Just Joined.", reply_to_message_id=reply,
                    parse_mode=ParseMode.HTML,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Nigga Dev just joined the group"
                )
                continue
            ...
            # Welcome Sudos
                ...
                update.effective_message.reply_video(
                    MAYOR_IMG, caption=f"Shhhhh!!!!!! A User with Mayor Disaster Just Joined. \nBe carefull", reply_to_message_id=reply,
                    parse_mode=ParseMode.HTML,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Retard Sudo user just joined the group"
                )
                continue
            ...
            # Welcome Support
                ...
                update.effective_message.reply_text(
                    "A User from the 9's just joined the group! stay alert!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"One of The 9's just joined the group"
                )
                continue
            ...
            # Welcome Whitelisted
                ...
                update.effective_message.reply_text(
                    " A special user Just joined the group!", reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Special user joined the chat"
                )
                continue
            ...
            # Welcome Tigers
                ...
                update.effective_message.reply_text(
                    "One of my Commanders just joined the group!", reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Commander joined the chat"
                )
                continue
